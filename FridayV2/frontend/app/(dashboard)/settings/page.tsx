...
export default function SettingsPage() {
  ...
  async function saveEdit(id: string) {
    await supabase.from("user_memory").update({ fact: editFact, updated_at: new Date().toISOString() }).eq("id", id);
    setEditId(null);
    loadMemory();
  }
                  ...
                  <div key={r.id} className="glass" style={{ padding: "14px 18px", display: "flex", alignItems: "flex-start", gap: 12 }}>
                    ...
                    {editId === r.id ? (
                      <>
                        ...
                        <button onClick={() => setEditId(null)} style={{ color: "var(--text-3)", fontSize: 12, background: "none", border: "none", cursor: "pointer" }}>Cancel</button>
                      </>
                    ) : (
                      <>
                        <p style={{ flex: 1, fontSize: 13, color: "var(--text-1)", lineHeight: 1.5 }}>{r.fact}</p>
                        <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--text-3)", flexShrink: 0, marginTop: 2 }}>{r.source}</span>
                        <button onClick={() => { setEditId(r.id); setEditFact(r.fact); }} className="btn-icon" style={{ fontSize: 12 }}>✎</button>
              ...
              return (
                <div key={w.id} className="glass" style={{ padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <span style={{ fontSize: 14, color: visible ? "var(--text-1)" : "var(--text-3)", fontFamily: "var(--font-space)", fontWeight: 500 }}>{w.label}</span>
                  </div>
                  <button
