...
export default function FinancePage() {
  ...
  async function addVar() {
    ...
    try { await apiFetch("/finance/variable", { method: "POST", body: JSON.stringify({ date: newVar.date, category: newVar.category, description: newVar.description, amount: parseFloat(newVar.amount) }) }); setNewVar({ date: new Date().toISOString().slice(0, 10), category: "", description: "", amount: "" }); load(); }
    catch (e) { alert(String(e)); }
  }
  async function saveEditVar() {
    if (!editVar) return;
    try { await apiFetch(`/finance/variable/${editVar._index}`, { method: "PUT", body: JSON.stringify({ date: editVar.date, category: editVar.category, description: editVar.description, amount: parseFloat(editVar.amount) }) }); setEditVar(null); load(); }
    catch (e) { alert(String(e)); }
  }
  async function deleteVar(idx: number) {
    try { await apiFetch(`/finance/variable/${idx}`, { method: "DELETE" }); load(); } catch (e) { alert(String(e)); }
  }
                ...
                <tr key={v._index}>
                  {editVar?._index === v._index ? (
                    ...
                    <>
                      ...
                      <td>{v.description}</td>
                      <td style={{ textAlign: "right", color: "var(--orange)", fontFamily: "var(--font-mono)" }}>{parseFloat(v.amount).toFixed(2)}</td>
                      <td style={{ textAlign: "right", whiteSpace: "nowrap" }}>
              ...
              {fixed.map((f) => (
                <tr key={f._index}>
                  {editFixed?._index === f._index ? (
                    ...
                    <>
                      <td>{f.item}</td>
                      <td style={{ textAlign: "right", color: "var(--cyan)", fontFamily: "var(--font-mono)" }}>{parseFloat(f.cost).toFixed(2)}</td>
