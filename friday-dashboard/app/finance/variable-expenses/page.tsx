...
const KNOWN_CATEGORIES = ["Groceries", "Family", "Self"];
...
function computeSummary(rows: ExpenseRow[]): MonthSummary {
...
export default function VariableExpensesPage() {
  ...
  const fetchMonth = useCallback((m: string) => {
    ...
    supabase
      .from("expenses")
      .select("id,date,category,description,recorder,amount")
      .eq("user_id", USER_ID)
      .gte("date", `${m}-01`)
      .lte("date", `${m}-${String(last).padStart(2, "0")}`)
      .order("date", { ascending: true })
      .then(({ data, error }) => {
        if (error) console.error("[VarExpenses] fetch error:", error);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`expenses_var_${month}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "expenses", filter: `user_id=eq.${USER_ID}` }, () => fetchMonth(month))
      .subscribe();
  ...
  async function saveEdit() {
    ...
    await supabase.from("expenses").update(updated).eq("id", editingId);
    setSummary((prev) => {
      if (!prev) return prev;
    ...
    <div className={`px-4 pb-3 flex flex-col gap-3 ${summary && summary.entries.length > 0 ? "border-b border-[#0d1e30]" : ""}`}>
      ...
      <div className="flex gap-2 justify-end">
        ...
        <button
          onClick={addExpense}
          disabled={saving}
          className="px-4 py-1.5 bg-[#00d4ff]/10 border border-[#00d4ff]/30 rounded-lg text-[11px] text-[#00d4ff] hover:bg-[#00d4ff]/20 transition-colors disabled:opacity-50"
        >
          {saving ? "Saving…" : "Save"}
        </button>
      </div>
        ...
        {loading ? (
          ...
          <div className="bg-[#0a1628] border border-[#1a3a5c] rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 pt-4 pb-3">
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#4a7a9b]">Transactions</p>
              {!adding && (
                <button
                  onClick={() => setAdding(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 border border-[#1a3a5c] rounded-lg text-xs text-[#4a7a9b] hover:border-[#00d4ff] hover:text-[#00d4ff] transition-colors"
                >
            ...
            <div className="px-4 py-8 text-center border-t border-[#0d1e30]">
              <p className="text-[#4a7a9b] text-sm">No expenses recorded for {monthDisplay(month)}.</p>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              <div className="bg-[#0a1628] border border-[#1a3a5c] rounded-xl p-4 text-center">
                <p className="text-[10px] text-[#4a7a9b] uppercase tracking-widest mb-1">Total Spent</p>
                <p className="text-lg font-mono font-bold text-[#00d4ff]">
                ...
                {summary.entries.map((e) =>
                  editingId === e.id ? (
                    ...
                    <div key={e.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-[#060e1c] transition-colors">
                      ...
                      {e.recorder === "Friday" && (
                        <span className="text-[9px] text-[#4a7a9b] border border-[#1a3a5c] rounded-full px-1.5 py-0.5 shrink-0">
                          Friday
                        </span>
                      )}
                      <span className={`text-xs font-mono text-[#00d4ff] shrink-0 ${show ? "" : "blur-sm select-none pointer-events-none"}`}>
                        {fmtAmount(e.amount)}
                      </span>
                      <div className="flex items-center gap-1 shrink-0 ml-1">
                        <button
                          onClick={() => startEdit(e)}
                          className="p-1.5 rounded text-[#2a4a6a] hover:text-[#00d4ff] hover:bg-[#00d4ff]/10 transition-colors"
                          title="Edit"
                        >
                          <EditIcon />
                        </button>
                        <button
                          ...
                          className="p-1.5 rounded text-[#2a4a6a] hover:text-red-400 hover:bg-red-400/10 transition-colors"
                          title="Delete"
                        >
                          <TrashIcon />
                        </button>
